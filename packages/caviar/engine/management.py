...
class ManagementResource:
	...
	def __init__(self, http_resource, data):
		...
		self.__command = data.get("command")
...
class ManagementExtraProperties:
	...
	def __init__(self, http_resource, data):
		...
		self.__system_properties = ManagementProperties(
			data.get("systemProperties", [])
		)
	...
	def __parameters(self, param_class, params_data):
		...
		return {
			param_name.replace("-", "_"): param_class(
				...
				param_data["type"],
				param_data.get("optional", "false") == "true",
...
class ManagementChildResources:
	...
	def keys(self):
		...
		yield from self.__children.keys()
...
class ManagementProperties:
	...
	def __init__(self, data):
		...
		self.__values = {
			prop["name"]: ( prop["defaultValue"], prop.get("description") )
			for prop in data
		}
	...
	def resolve(self, value):
		...
		if isinstance(value, str):
			interpolator = PropertyInterpolator(self.__values)
...
class PropertyInterpolator:
	...
	def __parse_text(self, c):
	
		if c == "$":
			return self.__parse_prop_name_begin
			
		self.__result.write(c)
		return self.__parse_text
		
	def __parse_prop_name_begin(self, c):
	
		if c == "{":
			return self.__parse_prop_name
			
		self.__result.write("$")
		self.__result.write(c)
		return self.__parse_text
		
	def __parse_prop_name(self, c):
	
		if c == "}":
			value, _ = self.__values[self.__prop_name.getvalue().strip()]
			
			interpolator = PropertyInterpolator(self.__values)
			self.__result.write(interpolator.interpolate(value))
			self.__prop_name.close()
			self.__prop_name = io.StringIO()
			return self.__parse_text
			
		self.__prop_name.write(c)
		return self.__parse_prop_name
		
	# TODO Test coverage
	def interpolate(self, value):
	
		self.__result = io.StringIO()
		self.__prop_name = io.StringIO()
		parse_fn = self.__parse_text
	
		for c in value:
			parse_fn = parse_fn(c)
		self.__result.seek(0)
		return self.__result.read()
